...
const uint8_t osd = 5;                     // with corresponding oversampling delay

float T, P;                                 // Set global variables for temperature and pressure
double baseline; // baseline pressure

void setupBMP180(System::State* state)
{
  //is this sensor connected and working?
  state->status_BMP180 = 0;
  ...
  int32_t b5;
  b5 = temperature();
  baseline = pressure(b5);
  state->p0_BMP180 = baseline;
  state->status_BMP180 = 1; // this sensor is working properly
}

void get_BMP180_data(System::State* state)
{
  int b5 = temperature();
  state->temperature_BMP180          = T;                    // Read and calculate temperature (T)
  state->pressure_BMP180             = pressure(b5);         // Read and calculate pressure (P)
  state->barometric_altitude_BMP180  = altitude(state->pressure_BMP180, baseline);  //relative altitude
}


String get_BMP180_data_str()
{
  int32_t b5;
  ...
  //relative altitude
  double alt = altitude(P, baseline);
  char buf[24];
  String retVal;
  retVal = dToStr(T, 8, 2, buf);
  retVal +=  " C, " ;
  retVal += dToStr(P, 8, 2, buf);
  retVal += " mbar";
  retVal += dToStr(alt, 8, 1, buf);
  retVal += " meter";
  return retVal;
